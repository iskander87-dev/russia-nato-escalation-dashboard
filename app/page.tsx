...
export default function Home() {
  ...
  return <main>
    ...
    <section className="hero">
      ...
      <article className="trend"><p className="eyebrow">Динамика и журнал</p><div className="trend-value"><ArrowUpRight/> Умеренное повышение <small>внутри уровня 3</small></div><div className="events"><span><b>12.02</b> Балтийская безопасность</span><span><b>02.09</b> Атрибуция Лейпцига</span><span className="now"><b>10.09</b> Подтверждение НАТО</span></div><p className="trend-note">Предыдущая оценка: 3,3 <b>↑ +0,3</b></p></article>
    </section>

    <section className="legend"><div className="legend-heading"><p className="eyebrow">Шкала эскалации</p><h2>Легенда уровней</h2></div><div className="legend-content"><div className="levels">{levels.map(([n,name,cls])=><div key={n} className={`level ${cls} ${n==='3'?'active':''}`} title={name}><b>{n}</b><span>{name}</span></div>)}</div><figure className="scale-art"><img src="/escalation-scale-compact.png" alt="Семь иллюстрированных ступеней: от спокойного наблюдения до широкомасштабного конфликта"/><figcaption>Визуальная шкала: сцены показывают характер риска, а не прогноз событий.</figcaption></figure></div></section>

    <section className="factors"><article className="rise"><h3><AlertTriangle/> Что повышает оценку</h3><ul><li>Германия официально атрибутировала России гибридную атаку на аэропорт Лейпциг/Галле.</li><li>Рост гибридной активности: кибер, саботаж, вмешательство.</li><li>Воздушные нарушения и повышенная готовность к риску.</li></ul></article><article className="restraint"><h3><ShieldCheck/> Что удерживает уровень ниже 4</h3><ul><li>Нет подтверждённых боевых действий или ракетных пусков по территории Альянса.</li><li>Нет подтверждённой серии кинетических перехватов.</li><li>Нет публично подтверждённого качественного скачка ядерной готовности.</li></ul></article></section>
    ...
    <section className="sources"><div><p className="eyebrow">Журнал проверок</p><h2>Ключевые события</h2></div><div className="source-list"><div><time>10.09.2026</time><p>НАТО подтвердило солидарность с Германией после атрибуции Лейпцига и продолжение мер сдерживания на восточном фланге.</p><Source id="berlin"/></div><div><time>02.09.2026</time><p>Правительство Германии возложило на Россию ответственность за гибридную атаку на аэропорт Лейпциг/Галле.</p><Source id="leipzig"/></div><div><time>18.06.2026</time><p>Регулярное заседание Nuclear Planning Group; открытые данные не подтверждают смену готовности российских сил.</p><Source id="nuclear"/></div></div></section>
