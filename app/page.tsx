...
export default function Home() {
  ...
  return <main>
    ...
    <section className="legend"><div><p className="eyebrow">Шкала эскалации</p><h2>Легенда уровней</h2></div><div className="legend-content"><div className="levels">{levels.map(([n,name,cls])=><div key={n} className={`level ${cls} ${n==='3'?'active':''}`} title={name}><b>{n}</b><span>{name}</span></div>)}</div><figure className="scale-art"><img src="/escalation-scale-illustrated.png" alt="Семь иллюстрированных ступеней: от спокойного наблюдения до широкомасштабного конфликта"/><figcaption>Визуальная шкала: сцены показывают характер риска, а не прогноз событий.</figcaption></figure></div></section>
